...
var sendImageToServer = () => {
  ...
  axios.post('/upload/fileResource', formData, {
    headers: {
...
var uploadResource = () => {
  var resource = $('input[name=prodImg]');
  console.log(resource[0].files[0].name);
  console.log(resource[0].files[0].size);
  console.log(resource[0].files[0].type);
  console.log(resource[0].files);

  var formData = new FormData();

  formData.append('prodImg', resource[0].files[0]);
  ...
  axios.post('/upload/resource', formData, {
    ...
    console.log(response);
    productImageUrl = response.data.file_path;
  }).catch((err) => { 'error while upload resource from client side' });
}